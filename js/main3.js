...
(function () {
  ...
  /* the fused poetry chapter: sticky viewport, scroll-driven stages */
  const pin = document.querySelector(".elements-pin");
  const pinViewport = pin ? pin.querySelector(".elements-pin__viewport") : null;
  const pinStages = pin ? Array.from(pin.querySelectorAll(".pin-stage")) : [];
  const pinMarks = pin ? Array.from(pin.querySelectorAll(".pin-progress__mark")) : [];
  const STAGE_IDS = ["water", "fire", "earth", "air"];
  ...
  function measure() {
    ...
    if (pin) {
      ...
      pinTop = r.top + y;
      pinRange = Math.max(1, pin.offsetHeight - pinViewport.offsetHeight);
    }
  }
  ...
  function applyPin(y) {
    if (!pin) return;
    const p = (y - pinTop) / pinRange;
    const far = p < -0.1 || p > 1.1;
    if (far && pinSettled) return;
    pinSettled = far;
    const stageF = 0.5 + clamp01(p) * 3;
    const idx = Math.min(3, Math.floor(stageF));
    for (let i = 0; i < pinStages.length; i++) {
      const dist = stageF - (i + 0.5);
      const o = clamp01((0.82 - Math.abs(dist)) / 0.42);
      const st = pinStages[i];
      st.style.setProperty("--o", o.toFixed(3));
      st.style.setProperty("--sp", (dist * -64).toFixed(1) + "px");
      st.style.visibility = o <= 0.001 ? "hidden" : "visible";
    }
    const inside = p >= -0.05 && p <= 1.02;
    pinMarks.forEach((m, i) => {
      const cur = inside && i === idx;
      m.classList.toggle("is-current", cur);
      if (cur !== m.hasAttribute("aria-current")) {
        if (cur) m.setAttribute("aria-current", "true");
        else m.removeAttribute("aria-current");
      }
    });
    if (inside) {
      if (root.dataset.scene !== STAGE_IDS[idx]) root.dataset.scene = STAGE_IDS[idx];
    } else if (root.dataset.scene) {
  ...
  pinMarks.forEach((m, i) => {
    m.addEventListener("click", () => {
      window.scrollTo({ top: pinTop + (i / 3) * pinRange, behavior: "smooth" });
    });
  });
